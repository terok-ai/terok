...
async def test_wizard_form_submit_returns_collected_dict() -> None:
    ...
    # Default path: the user did not open the "Override agents…" modal, so
    # ``agents`` is absent — the project inherits the global default
    # written via ``terok agents set``.
    assert "agents" not in app.result
...
@pytest.mark.asyncio
async def test_wizard_agents_button_default_label_is_override() -> None:
    """No override staged → button reads 'Override agents…' so users see the opt-in."""
    from textual.widgets import Button

    app = _WizardHost(WizardFormScreen())
    async with app.run_test() as pilot:
        await pilot.pause()
        label = str(app.screen.query_one("#wizard-form-agents", Button).label)
    assert label == "Override agents…"


@pytest.mark.asyncio
async def test_wizard_agents_button_label_shows_staged_selection() -> None:
    """A prefill ``agents`` value flips the button label to the staged value."""
    from textual.widgets import Button

    app = _WizardHost(WizardFormScreen(initial={"agents": "claude,vibe"}))
    async with app.run_test() as pilot:
        await pilot.pause()
        label = str(app.screen.query_one("#wizard-form-agents", Button).label)
    assert label == "Agents: claude,vibe"


@pytest.mark.asyncio
async def test_wizard_submit_includes_agents_only_when_overridden() -> None:
    """Form result carries ``agents`` only when the override modal returned a value."""
    ...
    async with app.run_test() as pilot:
        ...
        await pilot.pause()
    assert isinstance(app.result, dict)
    assert app.result["agents"] == "claude"


@pytest.mark.asyncio
async def test_wizard_modal_dismissal_updates_staged_override() -> None:
    """When the AgentsSelectScreen returns a value, the form captures it."""
    from textual.widgets import Button
    ...
    async with app.run_test() as pilot:
        ...
        screen = app.screen
        assert isinstance(screen, WizardFormScreen)
        # modal integration covered by test_agents_screen.py
        screen._on_agents_dismissed("all,-vibe")
        await pilot.pause()
        label = str(screen.query_one("#wizard-form-agents", Button).label)
    assert label == "Agents: all,-vibe"
