...
# ── _restore_shield_state / _drop_shield_on_creation ─────────


class TestRestoreShieldState:
    """Verify the extracted _restore_shield_state helper."""

    def test_noop_when_no_file(self, tmp_path: Path) -> None:
        """No state file means nothing to restore."""
        from terok.lib.orchestration.task_runners import _restore_shield_state

        with patch("terok.lib.orchestration.task_runners._shield_down_impl") as mock_down:
            _restore_shield_state("ctr", tmp_path)
        mock_down.assert_not_called()

    def test_noop_when_state_is_up(self, tmp_path: Path) -> None:
        """Saved 'up' state means the OCI hook is correct — do nothing."""
        from terok.lib.orchestration.task_runners import _restore_shield_state
    ...
    def test_restores_down(self, tmp_path: Path) -> None:
        """Saved 'down' triggers shield_down without allow_all."""
        from terok.lib.orchestration.task_runners import _restore_shield_state
    ...
    def test_restores_down_all(self, tmp_path: Path) -> None:
        """Saved 'down_all' triggers shield_down with allow_all=True."""
        from terok.lib.orchestration.task_runners import _restore_shield_state
    ...
    def test_warns_on_failure(self, tmp_path: Path) -> None:
        ...
        with (
            patch(
                "terok.lib.orchestration.task_runners._shield_down_impl",
                side_effect=RuntimeError("nft gone"),
            ),
            pytest.warns(match="shield restore"),
        ):
            _restore_shield_state("ctr", tmp_path)
...
class TestDropShieldOnCreation:
    """Verify the extracted _drop_shield_on_creation helper."""

    def test_drops_and_persists(self, tmp_path: Path) -> None:
        """Calls shield_down and writes 'down' state."""
        from terok.lib.orchestration.task_runners import _drop_shield_on_creation

        with patch("terok.lib.orchestration.task_runners._shield_down_impl") as mock_down:
            _drop_shield_on_creation("ctr", tmp_path)
        mock_down.assert_called_once_with("ctr", tmp_path)
        assert (tmp_path / "shield_desired_state").read_text().strip() == "down"
    ...
    def test_warns_on_failure(self) -> None:
        ...
        with (
            ...
            _drop_shield_on_creation("ctr", MOCK_TASK_DIR)


# ── _read_desired_shield_state / _write_desired_shield_state ──


class TestDesiredShieldStateIO:
    """Verify the low-level shield state file helpers."""

    def test_read_missing_file(self, tmp_path: Path) -> None:
        """Returns None when no state file exists."""
        from terok.lib.orchestration.task_runners import _read_desired_shield_state

        assert _read_desired_shield_state(tmp_path) is None

    @pytest.mark.parametrize("state", ["up", "down", "down_all"])
    def test_read_valid_states(self, tmp_path: Path, state: str) -> None:
        """Each valid state string is returned as-is."""
        from terok.lib.orchestration.task_runners import _read_desired_shield_state

        (tmp_path / "shield_desired_state").write_text(f"{state}\n")
        assert _read_desired_shield_state(tmp_path) == state

    def test_read_invalid_content(self, tmp_path: Path) -> None:
        """Garbage content returns None (not a valid state)."""
        from terok.lib.orchestration.task_runners import _read_desired_shield_state

        (tmp_path / "shield_desired_state").write_text("bogus\n")
        assert _read_desired_shield_state(tmp_path) is None
    ...
    def test_read_empty_file(self, tmp_path: Path) -> None:
        ...
        (tmp_path / "shield_desired_state").write_text("")
        assert _read_desired_shield_state(tmp_path) is None

    def test_write_and_read_roundtrip(self, tmp_path: Path) -> None:
        """Write then read returns the same state."""
        from terok.lib.orchestration.task_runners import (
            _read_desired_shield_state,
            _write_desired_shield_state,
        )

        _write_desired_shield_state(tmp_path, "down_all")
