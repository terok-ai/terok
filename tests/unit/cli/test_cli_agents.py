...
def test_set_writes_selection_after_validation(
    ...
    with (
        patch("terok.lib.integrations.executor.get_roster", return_value=_fake_roster()),
        patch(
            "terok.lib.integrations.executor.parse_agent_selection",
            side_effect=lambda raw: raw,
        ),
        patch(
            "terok.lib.integrations.executor.set_global_image_agents",
...
def test_set_rejects_unknown_agent(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    with (
        ...
        patch("terok.lib.integrations.executor.set_global_image_agents") as write_mock,
