...
**Pre-1.0 external dependencies:** pin third-party `0.x` dependencies (e.g. `ruamel.yaml`, `agent-client-protocol`, `sqlcipher3`, `asyncvarlink`) to an **exact patch** (`asyncvarlink==0.3.2`), not a floating `>=0.3.1,<0.4.0` range. Pre-1.0 projects routinely ship breaking API changes in patch/minor bumps — a floating range lets a fresh install (one that doesn't build from `poetry.lock`) pull an incompatible release the pinned lockfile never exercised. This bit us in terok-ai/terok#1110, where `asyncvarlink` 0.3.2 broke `terok-clearance` at import. Bump these pins deliberately and re-run the suite. This applies stack-wide to every terok package; our own `terok-*` siblings are exempt (their APIs are ours to keep compatible, and they follow the sibling version-sync rules above).
