...
In the per-container-supervisor model every container hosts its own
hub + verdict server in the supervisor process — there is no host-side
``HubService`` / ``NotifierService`` daemon, so the legacy installers
and unit-version probes are gone.  Operator UIs that previously talked
to a single global hub socket now multiplex across the per-container
sockets via
