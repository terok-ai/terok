...
Every ``terok_executor`` / ``terok_sandbox`` / ``terok_clearance`` /
``terok_shield`` import in the codebase routes through this package.
When a sibling release renames, splits, or relocates a symbol, only the
corresponding adapter needs to change — the rest of terok keeps reading
the same ``terok.lib.integrations.X`` name.

The boundary is enforced by ``.importlinter``'s ``*-boundary``
contracts: outside this package, importing a sibling wheel directly is
