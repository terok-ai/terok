...
vault, and the shield hook pair (which now installs both nft and
bridge hooks together, so a single shield-uninstall covers the event
wire too).  There is no host gate daemon to uninstall — the gate runs
inside each container's supervisor and leaves nothing behind on the
host.  Terok's wrapper is a thin delegating call.
