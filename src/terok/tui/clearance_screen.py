...
class ClearanceScreen(screen.Screen[None]):
    ...
    def on_app_focus(self, _event: events.AppFocus) -> None:
        ...
        The multi-socket subscriber carries no per-socket reconnect
        back-off for a refocus to cut short — its rescan loop reconciles
        disappeared sockets on its own short interval, and any child that
        fails on connect is dropped + retried on the next rescan
        automatically.
        """
        del _event
    ...
    async def on_unmount(self) -> None:
        ...
        try:
            ...
            _log.debug("Failed to stop subscriber: %s", exc)
        finally:
            if self._notifier:
                try:
                    await self._notifier.disconnect()
                except Exception as exc:
                    _log.debug("Failed to disconnect notifier: %s", exc)
